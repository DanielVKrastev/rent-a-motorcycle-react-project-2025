...
import { useDeleteRequest } from "../../../../api/customerRequestApi";

export default function CustomerRequestDelete( {
    request,
    setIsOpen,
    handleDeleteLocal,
}) {
    const [showMessageToast, setMessageShowToast] = useState(false);
    const { deleteCustomerReqeust } = useDeleteRequest();

    async function deleteRequestHandler(){
        try{

            await deleteCustomerReqeust(request._id);
            handleDeleteLocal(request._id);
            setIsOpen(false);
        }catch(err){
